...
빌드 시 Obsidian 콘텐츠가 자동으로 변환됩니다:

| Obsidian 문법 | 변환 결과 |
|--------------|----------|
| `[[노트 제목]]` | 내부 링크 (HTML anchor) |
| `[[노트\|표시텍스트]]` | 커스텀 텍스트 링크 |
| `> [!NOTE] 제목` | 스타일된 노트 박스 |
| `> [!TIP]`, `[!WARNING]` 등 | 12가지 콜아웃 스타일 |
| 코드 블록 | 언어 배지 + 복사 버튼 + 라인 수 |
| 3개 이상 헤딩 | 자동 목차(TOC) 생성 |
...
지원: NOTE, TIP, WARNING, DANGER, INFO, IMPORTANT, EXAMPLE, QUOTE, SUCCESS, FAILURE, BUG, ABSTRACT
...
→ 언어 배지 (JavaScript) + 복사 버튼 + "1 line" 표시
